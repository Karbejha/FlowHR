...
const EditUserForm: React.FC<EditUserFormProps> = ({ employee, onSuccess, onCancel }) => {
  ...
  const { user, token } = useAuth();  const [formData, setFormData] = useState({
    ...
    salaryInfo: {
      ...
      allowances: {
        transportation: employee.salaryInfo?.allowances?.transportation || 0,
        housing: employee.salaryInfo?.allowances?.housing || 0,
        food: employee.salaryInfo?.allowances?.food || 0,
        mobile: employee.salaryInfo?.allowances?.mobile || 0,
        other: employee.salaryInfo?.allowances?.other || 0,
      },
      taxRate: employee.salaryInfo?.taxRate || 0,
      socialInsuranceRate: employee.salaryInfo?.socialInsuranceRate || 0,
      healthInsuranceRate: employee.salaryInfo?.healthInsuranceRate || 0,
      overtimeRate: employee.salaryInfo?.overtimeRate || 1.5,
    }
  });
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-purple-600 to-purple-700 dark:from-purple-700 dark:to-purple-800 px-6 py-4">
        <div className="flex items-center space-x-3">
          ...
          <h2 className="text-xl font-bold px-2 text-white">{t('employees.editEmployee')}</h2>
        </div>
      </div>
