"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getLeaveBalance = exports.cancelLeaveRequest = exports.updateLeaveStatus = exports.getPendingLeaveRequests = exports.getMyLeaveRequests = exports.submitLeaveRequest = void 0;
const Leave_1 = require("../models/Leave");
const User_1 = require("../models/User");
const userUtils_1 = require("../utils/userUtils");
const submitLeaveRequest = async (req, res) => {
    try {
        const { leaveType, startDate, endDate, reason } = req.body;
        // Convert leaveType to enum value
        const leave = new Leave_1.Leave({
            employee: req.user._id,
            leaveType: leaveType.toLowerCase(),
            startDate: new Date(startDate),
            endDate: new Date(endDate),
            reason,
            status: Leave_1.LeaveStatus.PENDING
        });
        // Calculate total days
        const totalDays = leave.calculateTotalDays();
        // Validate leave balance before saving
        const hasBalance = await leave.validateLeaveBalance();
        if (!hasBalance) {
            res.status(400).json({ error: 'Insufficient leave balance' });
            return;
        }
        await leave.save();
        await leave.populate('employee', 'firstName lastName email');
        res.status(201).json(leave);
    }
    catch (error) {
        console.error('Leave request error:', error);
        res.status(400).json({ error: error instanceof Error ? error.message : 'Error submitting leave request' });
    }
};
exports.submitLeaveRequest = submitLeaveRequest;
const getMyLeaveRequests = async (req, res) => {
    try {
        const leaves = await Leave_1.Leave.find({ employee: req.user._id })
            .sort({ createdAt: -1 })
            .populate('employee', 'firstName lastName')
            .populate('approvedBy', 'firstName lastName');
        res.json(leaves);
    }
    catch (error) {
        res.status(500).json({ error: 'Error fetching leave requests' });
    }
};
exports.getMyLeaveRequests = getMyLeaveRequests;
const getPendingLeaveRequests = async (req, res) => {
    try {
        let query = {
            status: Leave_1.LeaveStatus.PENDING
        };
        // If manager, only show their team's requests
        if (req.user.role === User_1.UserRole.MANAGER) {
            const teamMembers = await (0, userUtils_1.getTeamMembers)(req.user._id);
            query.employee = {
                $in: teamMembers.map(member => member._id)
            };
        }
        const leaves = await Leave_1.Leave.find(query)
            .sort({ createdAt: -1 })
            .populate('employee', 'firstName lastName department')
            .populate('approvedBy', 'firstName lastName');
        res.json(leaves);
    }
    catch (error) {
        res.status(500).json({ error: 'Error fetching pending leave requests' });
    }
};
exports.getPendingLeaveRequests = getPendingLeaveRequests;
const updateLeaveStatus = async (req, res) => {
    try {
        const { leaveId } = req.params;
        const { status, approvalNotes } = req.body;
        const leave = await Leave_1.Leave.findById(leaveId)
            .populate('employee', 'firstName lastName email leaveBalance');
        if (!leave) {
            res.status(404).json({ error: 'Leave request not found' });
            return;
        }
        // Check if user has permission to approve/reject
        if (req.user.role === User_1.UserRole.MANAGER) {
            const teamMembers = await (0, userUtils_1.getTeamMembers)(req.user._id);
            const teamMemberIds = teamMembers.map(member => member.id);
            const employeeId = leave.employee._id.toString();
            if (!teamMemberIds.includes(employeeId)) {
                res.status(403).json({ error: 'Not authorized to update this leave request' });
                return;
            }
        }
        // Only deduct balance when status changes to approved
        if (status === Leave_1.LeaveStatus.APPROVED && leave.status !== Leave_1.LeaveStatus.APPROVED) {
            const user = await User_1.User.findById(leave.employee._id);
            if (!user) {
                res.status(404).json({ error: 'Employee not found' });
                return;
            }
            // Initialize leave balance if not set
            if (!user.leaveBalance) {
                user.leaveBalance = {
                    annual: 0,
                    sick: 0,
                    casual: 0
                };
            }
            const balanceType = leave.leaveType.toLowerCase();
            if (user.leaveBalance[balanceType] < leave.totalDays) {
                res.status(400).json({ error: 'Insufficient leave balance' });
                return;
            }
            // Deduct the leave balance
            user.leaveBalance[balanceType] = user.leaveBalance[balanceType] - leave.totalDays;
            await user.save();
        }
        // Restore balance if previously approved leave is now rejected/cancelled
        if (leave.status === Leave_1.LeaveStatus.APPROVED &&
            (status === Leave_1.LeaveStatus.REJECTED || status === Leave_1.LeaveStatus.CANCELLED)) {
            const user = await User_1.User.findById(leave.employee._id);
            if (user === null || user === void 0 ? void 0 : user.leaveBalance) {
                const balanceType = leave.leaveType.toLowerCase();
                user.leaveBalance[balanceType] = user.leaveBalance[balanceType] + leave.totalDays;
                await user.save();
            }
        }
        leave.status = status;
        leave.approvalNotes = approvalNotes;
        leave.approvedBy = req.user._id;
        leave.approvalDate = new Date();
        await leave.save();
        res.json(leave);
    }
    catch (error) {
        res.status(400).json({ error: 'Error updating leave status' });
    }
};
exports.updateLeaveStatus = updateLeaveStatus;
const cancelLeaveRequest = async (req, res) => {
    try {
        const { leaveId } = req.params;
        const leave = await Leave_1.Leave.findById(leaveId);
        if (!leave) {
            res.status(404).json({ error: 'Leave request not found' });
            return;
        }
        // Only allow cancellation of own pending requests
        if (leave.employee.toString() !== req.user._id.toString() ||
            leave.status !== Leave_1.LeaveStatus.PENDING) {
            res.status(403).json({ error: 'Cannot cancel this leave request' });
            return;
        }
        leave.status = Leave_1.LeaveStatus.CANCELLED;
        await leave.save();
        res.json(leave);
    }
    catch (error) {
        res.status(400).json({ error: 'Error cancelling leave request' });
    }
};
exports.cancelLeaveRequest = cancelLeaveRequest;
const getLeaveBalance = async (req, res) => {
    try {
        const user = await User_1.User.findById(req.user._id).select('leaveBalance');
        if (!user) {
            res.status(404).json({ error: 'User not found' });
            return;
        }
        res.json(user.leaveBalance);
    }
    catch (error) {
        res.status(500).json({ error: 'Error fetching leave balance' });
    }
};
exports.getLeaveBalance = getLeaveBalance;
